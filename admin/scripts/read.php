...
function getSingleMovie($id) {
    $pdo = Database::getInstance()->getConnection();
    // TODO: finish the line with a proper SQL that only fetch movie for the given id
    $querySingle = 'SELECT * FROM tbl_movies WHERE movies_id = "' . $id . '"';
    $runSingle = $pdo->query($querySingle);

    if ($runSingle) {
        $movie = $runSingle->fetchAll(PDO::FETCH_ASSOC);
        return $movie;
    } else {
        return 'There was a problem accessing this info' . $id;
    }
}
